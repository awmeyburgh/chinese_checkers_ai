...
class PlayerBoard{
  ...
  public reset(enabled: boolean){
      if(enabled){
          this.pieces = reactive(new Group(this.base.points))
      }
      else{
          this.pieces = null
      }
...
export function useBoard() {
  const setPlayers = (playersConfig: Array<{ enabled: boolean, player: number, color: string, controller: string }>) => {
    playersConfig.forEach(playerConfig => {
      const playerBoard = board.players.find(p => p.id === playerConfig.player - 1);
      if (playerBoard) {
        playerBoard.reset(playerConfig.enabled);
      }
    });
  };
